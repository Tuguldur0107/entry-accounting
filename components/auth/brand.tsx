...
// Geometric mark — square with stacked accent lines
export function EAMark({ size = 32 }: { size?: number }) {
  const r = size * 0.18;
  return (
    <svg width={size} height={size} viewBox="0 0 40 40" aria-label="Гоёл Кашмер ХХК">
...
export function EAWordmark({ size = 18 }: { size?: number }) {
  return (
    ...
    >
      Гоёл <em style={{ fontStyle: 'italic', fontWeight: 400 }}>Кашмер</em>{' '}
      <span style={{ fontSize: '0.62em', fontWeight: 400, color: 'var(--ea-text-3)', letterSpacing: '0.04em' }}>
        ХХК
      </span>
    </span>
  );
