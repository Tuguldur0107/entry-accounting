// MCP (Model Context Protocol) SERVER — Entry Accounting-ийн tool давхаргыг
// Claude Code зэрэг гадны MCP клиентэд нээнэ.
//
//   claude mcp add --transport http entry-accounting https://<domain>/api/mcp \
//     --header "Authorization: Bearer <token>"
//
// Дизайн:
//   - Streamable HTTP, STATELESS горим — хүсэлт бүр бие даасан JSON-RPC
//     POST, хариу нь энгийн application/json (session/SSE шаардлагагүй).
//   - Нэвтрэлт: Bearer Personal Access Token (Тохиргоо → MCP холболт).
//     Token нь sha256 hash-аараа api_tokens-д хадгалагддаг.
//   - Tools = чатын agent-тай ЯГ ИЖИЛ давхарга (lib/ai/tools.ts) — ноорог
//     -first, 10 сая ₮ хязгаар, периодын хамгаалалт бүгд үйлчилнэ.
//   - Server action доторх auth() дуудлагууд runAsUser()-ийн ачаар token-ий
//     эзний session мэт ажиллана (lib/auth.ts).
...
/** Bearer token → userId (hash тулгалт + lastUsedAt тэмдэглэнэ). */
...
export async function POST(request: Request) {
  ...
  if (!userId) {
    return Response.json(
      { error: "Token буруу эсвэл хүчингүй — Тохиргоо → MCP холболт хэсгээс шинээр үүсгэнэ үү" },
