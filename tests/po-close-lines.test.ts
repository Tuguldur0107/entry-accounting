...
test("зөвхөн нэг тал бичигдсэн байхад ч мөр гарна (нэхэмжлэх ирээгүй)", () => {
  const lines = build(-5_000_000, 0);
  assert.equal(lines.length, 2);
  assert.equal(lines[0].accountNumber, buildCode("14000099"));
  assert.equal(lines[0].debit, "5000000");
  // Бүх зөрүү нь ханшийн гарз БИШ — олз тал руу (apDebit − invCredit < 0).
  assert.equal(lines[1].accountNumber, buildCode("51800001"));
  assert.equal(lines[1].credit, "5000000");
});
