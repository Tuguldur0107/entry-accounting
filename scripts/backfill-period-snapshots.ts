// Хаагдсан тайлант үеүдэд GL (account_period_balances), кассын
// (cash_account_period_balances), бараа материалын (inventory_period_balances)
// snapshot ДУТУУ бол нөхөж бичнэ. Идемпотент —
...
async function main() {
  ...
  const { writeInventoryPeriodSnapshot } = await import("../lib/inventory/period-snapshot");
  ...
  for (const period of closed) {
    // Бараа материалын snapshot нь бүх үлдэгдэл 0 үед 0 мөртэй — тэр үед
    // дахин бичих нь идемпотент (delete + insert), хор хөнөөлгүй.
    const [{ gl, cash, inv }] = (await db.execute(sql`
      select
        ...
        (select count(*) from inventory_period_balances i where i.organization_id = ${period.organizationId} and i.period_code = ${period.code})::int as inv
    `)) as unknown as { gl: number; cash: number; inv: number }[];
    if (gl > 0 && cash > 0 && inv > 0) continue;
    await db.transaction(async (tx) => {
      ...
      if (inv === 0)
        await writeInventoryPeriodSnapshot(tx, {
          orgId: period.organizationId,
          userId: period.userId,
          code: period.code,
          endDate: period.endDate,
    ...
    written += 1;
    console.log(`✓ ${period.organizationId.slice(0, 8)} ${period.code} — gl:${gl === 0 ? "бичив" : "байсан"} cash:${cash === 0 ? "бичив" : "байсан"} inv:${inv === 0 ? "бичив" : "байсан"}`);
  }
