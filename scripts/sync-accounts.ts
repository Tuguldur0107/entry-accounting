...
async function main() {
  ...
  const existing = await db.query.chartOfAccounts.findMany({ where: eq(schema.chartOfAccounts.userId, userId) });
  ...
  await db.insert(schema.chartOfAccounts).values(toAdd.map((a) => ({ userId, number: a.number, name: a.name })));
